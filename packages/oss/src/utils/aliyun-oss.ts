import { TAliyunOSSOption, OSSInterface } from '../type'
...
export class AliyunOss implements OSSInterface {
  ossOption: TAliyunOSSOption
  client: OSS

  constructor(options: TAliyunOSSOption) {
    this.ossOption = options
    this.client = new OSS(this.ossOption)
  }
  ...
  protected async doUploadFile(localFilePath: string, remoteFilePath: any) {
    ...
    try {
      ...
      console.error('Upload Error ', remoteFilePath, error)
    }
  }
